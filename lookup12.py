...
# Purpose : add red_green function and bottom
from Tkinter import * 
import requests
import json
import os
...
header_name = Label(root, text="Name", bg="white" )
header_name.grid(row=0, column=0, sticky=N+S+E+W)

header_rank = Label(root, text="Rank", bg="silver" )
header_rank.grid(row=0, column=1, sticky=N+S+E+W)

header_current_price = Label(root, text="Current Price", bg="white" )
header_current_price.grid(row=0, column=2, sticky=N+S+E+W)

header_price_paid = Label(root, text="Price Paid", bg="silver")
header_price_paid.grid(row=0, column=3, sticky=N+S+E+W)

header_profit_loss_per = Label(root, text="Profit/Loss Per", bg="white")
header_profit_loss_per.grid(row=0, column=4, sticky=N+S+E+W)

header_1_hr_change = Label(root, text="1 HR Change", bg="silver" )
header_1_hr_change.grid(row=0, column=5, sticky=N+S+E+W)

header_24_hr_change = Label(root, text="24-Hour Change", bg="white" )
header_24_hr_change.grid(row=0, column=6, sticky=N+S+E+W)

header_7_day_change = Label(root, text="7-Day Change", bg="silver" )
header_7_day_change.grid(row=0, column=7, sticky=N+S+E+W)

header_current_value = Label(root, text="Current Value", bg="white" )
header_current_value.grid(row=0, column=8, sticky=N+S+E+W)

header_profit_loss_total = Label(root, text="Profit/Loss Total", bg="silver" )
...
def lookup():
    ...
    portfolio_profits = Label(root, text="P/L: ${0:.2f}".format(float(portfolio_profit_loss)), bg="gray", fg=red_green(float(portfolio_profit_loss)) )
    ...
    #total_current_value_output = Label(root, text="total current value : ${0:.2f}".format(float(total_current_value)), bg="gray", fg=red_green(float(total_current_value)) )
    #total_current_value_output.grid(row=row_count +1, column=1, sticky=W, padx=10, pady =10)
    api = ""
    update_button = Button(root, text="Refresh Prices", command=lookup)
